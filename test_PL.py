...
def test(args):
    ...
    for ix, protein in enumerate(test_dataset):
        ...
        if args.ContGreedy:
            ...
            S_sample = model.sample(X, chain_encoding_all, residue_idx, mask, S, args.fix, args.ContGreedy, args.fix_positions)
    ...
    with open("generated_fasta/" + str(entry["name"]) +".fasta", "w") as f:
        for entry in dataSet:
            f.write(">" + str(entry["name"]) + " Identity:" + str(entry["Identity"]) + "\n" + str(entry["Designed_sequence"]) + "\n\n")
            
    """with open("5U4Y_C_10.josnl", "w") as f:
        for entry in dataSet:
            f.write(json.dumps(entry) + "\n")"""

    """length = len(dataSet)
    if args.ContGreedy:
        with open("5U4Y_C" + str(length) + ".jsonl", "w") as f:
            for entry in dataSet:
                f.write(json.dumps(entry) + "\n")"""
        
    """else:
        with open("g_CASP14_N" + str(length) + ".jsonl", "w")  as f:
            for entry in dataSet:
                f.write(json.dumps(entry) + "\n")
                with open("ProteinA_fix_5.fasta", "w") as f:
                    f.write(">ProteinA_fix_5 " + "Identity:" + str(identity) + "\n")
                    f.write(designed_seq)"""
if __name__ == "__main__":
    ...
    argparser.add_argument("--checkpoint_path", type=str, default="/home/wangxinglong/model/MPNNMRF/model_110.pth", help="Path of model")
    ...
    # add json path that for  inference
    ...
    argparser.add_argument("--pdb_path", type = str, 
                          default = "/home/wangxinglong/MPNN_dataset/5U4Y_C.pdb", 
                          help    = "Path of proteinA structure information file with the json format")
